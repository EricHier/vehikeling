  ...
  <PopupContainer :onFormClick="onFormClick" :onFormKeydown="onFormKeydown">
    <div>
      ...
      <form v-on:submit="pushNewCar" v-if="!disabled">
        <h1 class="custom-title" v-if="Object.entries(car).length === 0">Fahrzeug hinzufügen</h1>
        <h1 class="custom-title" v-if="Object.entries(car).length !== 0">Fahrzeug bearbeiten</h1>

        <Input v-on:newValue="newName" v-bind:default="name" v-bind:label="'Spitzname des Fahrzeugs'"
               v-bind:is-big="false"
               v-bind:required="false" v-bind:image="false" v-bind:disabled="disabled"/>

        <Input v-on:newValue="newHersteller" v-bind:default="hersteller" v-bind:label="'Hersteller des Fahrzeugs*'"
               v-bind:is-big="false"
               v-bind:required="true" v-bind:image="false" v-bind:disabled="disabled"/>

        <Input v-on:newValue="newModel" v-bind:default="model" v-bind:label="'Model des Fahrzeugs*'"
               v-bind:is-big="false"
               v-bind:required="true" v-bind:image="false" v-bind:disabled="disabled"/>

        <Input v-on:newValue="newColor" v-bind:default="color" v-bind:label="'Farbe des Fahrzeugs'"
               v-bind:is-big="false"
               v-bind:required="false" v-bind:image="false " v-bind:disabled="disabled"/>

        <Input v-on:newValue="newFahrgestellnummer" v-bind:default="fahrgestellnummer"
               v-bind:label="'Fahrgestellnummer des Fahrzeugs'"
        ...
        <Input v-bind:label="'Bild deines Fahrzeugs (max. 2 MB)*'" v-bind:secondline="tagUpload"
               v-on:loadImage="loadImage"
               v-bind:is-big="false" v-bind:required="true" v-bind:image="true" v-bind:disabled="disabled"
               v-if="images === null"/>

        <input v-bind:disabled="disabled" class="custom-button my-2" type="submit"
               name="Speichern"/>
        <button v-bind:disabled="disabled" class="custom-button" v-on:click="closeAddCar">Schließen</button>
      </form>
    </div>
  </PopupContainer>
</template>
<script>
  ...
  import Error from "../gui-components/Error";
  import PopupContainer from "../gui-components/PopupContainer";

  export default {
    name: 'AddCar',
    components: {Error, Input, Loading, PopupContainer},
    props: ["car"],
    data() {
      return {
        name: "",
